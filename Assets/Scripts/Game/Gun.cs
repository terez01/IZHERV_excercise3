...
public class Gun : MonoBehaviour
{
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            ...
            {
                // Calculate a random spread within the specified range
                float spreadAngle = UnityEngine.Random.Range(-2 * shotgunSpread, 2 * shotgunSpread);

                // Apply the spread to the director's euler angles
                Vector3 spreadEulerAngles = new Vector3(director.eulerAngles.x, director.eulerAngles.y + spreadAngle,
                    director.eulerAngles.z);

                // Convert the euler angles to a quaternion
                Quaternion spreadQuaternion = Quaternion.Euler(spreadEulerAngles);

                // Spawn a bullet with the spread quaternion
                SpawnBullet(director.position, spreadQuaternion);
